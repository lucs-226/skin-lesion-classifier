...
\begin{itemize}
    ...
    \item \textbf{Photometric Distortions:} Application of `ColorJitter` to randomly perturb brightness, contrast, and saturation.
\end{itemize}
...
[cite_start]To address this, we implement a custom \textbf{Focal Loss}[cite: 52]. Let $p_t$ be the model's estimated probability for the target class, derived from the computed Cross-Entropy term as $p_t = \exp(-CE(p, y))$. Our loss function is formulated as:
...
\begin{itemize}
    ...
    \item \textbf{Implementation:} We set `reduction='mean'` to normalize the loss across the batch, ensuring stable gradient updates for the fine-tuned backbone.
\end{itemize}
